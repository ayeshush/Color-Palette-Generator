...
var currentColor = "skyblue",
    currentType = "analogous",
    currentCount = 6,
    imageColors = [];

// FUNCTION TO GENERATE DIFFERENT PALETTES
function generateAnalogousPalette(hsl, count) {
    const palette = [];
    const [hue, saturation, lightness] = hsl;

    for (var i = 0; i < count; i++) {
        var newHue = (hue + 30 * i) % 360;
...
function generateMonochromaticPalette(hsl, count) {
    ...
    for (var i = 0; i < count; i++) {
        var newLightness = (lightness + 10 * i) % 100;
...
function generateShadesPalette(hsl, count) {
    ...
    for (var i = 0; i < count; i++) {
        var newSaturation = (saturation + 10 * i) % 100;
...
function generatePaletteHTML(type, container) {
    ...
    if (!hsl && type !== "image-colors") return;
    var palette = [];
    container.innerHTML = "";
...
function getHslFromColor(color) {
    ...
    if (isValidColor(color)) {
        ...
        var styles = window.getComputedStyle(temp, null);
        var rgb = styles.getPropertyValue("color");
